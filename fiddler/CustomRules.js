...
class Handlers {
	...
	//}
	public static RulesOption("Enable Unpacker", "f2e tools")
	var m_Unpacker : boolean = false;

	// Removes HTTP-caching related headers and specifies "no-cache" on requests and responses
	public static RulesOption("&Disable Caching", "f2e tools")
	var m_DisableCaching : boolean = false;

	public static RulesOption("Assets Proxy", "f2e tools")
	var m_AssetsProxy : boolean = false;

	public static var pathFrom : String = FiddlerObject.prompt('输入Assets路径前缀', "http://g.tbcdn.cn/ju/life/1.0.2", "Assets Proxy");
	public static var pathTo : String = FiddlerObject.prompt('输入目标前缀', "http://127.0.0.1/gitlab/life/src", "Assets Proxy");
	...
	static function OnBeforeRequest(oSession : Session) {
		...
		if (m_AssetsProxy && oSession.uriContains(pathFrom)) {
			oSession.fullUrl = oSession.fullUrl.Replace(pathFrom, pathTo);
			oSession.fullUrl = oSession.fullUrl.Replace("-min.", ".");
			oSession["ui-color"] = "red";
			oSession["ui-bold"] = "true";
			FiddlerObject.log("代理的url ：" + oSession.fullUrl);
		}
